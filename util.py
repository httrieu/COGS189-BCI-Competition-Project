...
def A3Load():
    # Set A3 directory
    DIR = os.path.join(os.path.expanduser("~"), 'F:/COGS189-BCI-Competition-Project/BCICIV_2a/')

    # Load the train data and format it
    EEGL_train = pickle.load(open(DIR+'tr_EEGL_1.pkl', "rb"))['L'] # (9, 11)
    EEGR_train = pickle.load(open(DIR+'tr_EEGR_1.pkl', "rb"))['R'] # (9, 11)
    for i in range(2,10):
        EEGL_train = np.vstack((EEGL_train,pickle.load(open(DIR+'tr_EEGL_'+str(i)+'.pkl', "rb"))['L']))
        EEGR_train = np.vstack((EEGR_train,pickle.load(open(DIR+'tr_EEGR_'+str(i)+'.pkl', "rb"))['R']))

    # Load labeled eval data and format it
    EEGL_eval = pickle.load(open(DIR+'ev_EEGL_1.pkl', "rb"))['L'] # (9, 11)
    EEGR_eval = pickle.load(open(DIR+'ev_EEGR_1.pkl', "rb"))['R'] # (9, 11)
    for i in range(2,10):
        EEGL_eval = np.vstack((EEGL_eval,pickle.load(open(DIR+'ev_EEGL_'+str(i)+'.pkl', "rb"))['L']))
        EEGR_eval = np.vstack((EEGR_eval,pickle.load(open(DIR+'ev_EEGR_'+str(i)+'.pkl', "rb"))['R']))

    # Load unlabeled eval data and format it
    EEG_eval = pickle.load(open(DIR+'ev_EEG_1.pkl', "rb"))['E'] # (9, 11)
    for i in range(2,10):
        EEG_eval = np.vstack((EEG_eval,pickle.load(open(DIR+'ev_EEG_'+str(i)+'.pkl', "rb"))['E']))
